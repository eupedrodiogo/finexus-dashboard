...
import { calculateAccountBalance, getOwnerLabel } from '../utils';
...
export const AccountsView: React.FC<AccountsViewProps> = ({ data, onUpdate, isMobile, isPedro = true }) => {
            ...
            <header className="mb-8 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    <h1 className="text-2xl font-bold text-slate-800 dark:text-white flex items-center gap-2">
                        ...
                        {isPedro ? 'Minhas Contas' : 'Contas Bancárias'}
                    </h1>
                    <p className="text-slate-600 dark:text-slate-400 mt-1">
                        {isPedro ? 'Gerencie o saldo das suas contas bancárias e carteiras.' : 'Visão consolidada de saldos e ativos financeiros.'}
                    </p>
                </div>
                <button
                ...
                >
                    <Plus size={20} />
                    {isPedro ? 'Nova Conta' : 'Adicionar Conta'}
                </button>
            </header>
                ...
                {groupedAccounts.pedro.length > 0 && (
                    <section>
                        <div className="flex items-center gap-2 mb-4 bg-slate-100 dark:bg-slate-800/50 p-2 rounded-lg w-fit pr-4">
                            ...
                            <h2 className="font-semibold text-slate-700 dark:text-slate-300">Contas {getOwnerLabel('pedro', isPedro)}</h2>
                        </div>
                ...
                {groupedAccounts.izabel.length > 0 && (
                    <section>
                        <div className="flex items-center gap-2 mb-4 bg-slate-100 dark:bg-slate-800/50 p-2 rounded-lg w-fit pr-4">
                            ...
                            <h2 className="font-semibold text-slate-700 dark:text-slate-300">Contas {getOwnerLabel('izabel', isPedro)}</h2>
                        </div>
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                            ...
                            <div className="space-y-4">
                                <div>
                                    ...
                                    <textarea
                                        ...
                                        className="w-full bg-slate-50 dark:bg-slate-700/50 border border-slate-200 dark:border-slate-600 rounded-xl px-4 py-3 outline-none focus:ring-2 focus:ring-indigo-500 dark:text-white resize-none"
                                        rows={2}
                                    />
                                </div>
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <select
                                            ...
                                            className="w-full bg-slate-50 dark:bg-slate-700/50 border border-slate-200 dark:border-slate-600 rounded-xl px-4 py-3 outline-none focus:ring-2 focus:ring-indigo-500 dark:text-white"
                                        >
                                            <option value="pedro">{getOwnerLabel('pedro', isPedro)}</option>
                                            <option value="izabel">{getOwnerLabel('izabel', isPedro)}</option>
